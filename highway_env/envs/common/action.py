...
class L0Action(ActionType):
  """Action type for the L0 agents."""
  ...
  def space(self) -> spaces.Space:
    """The action space."""
    return spaces.Dict({
        "acceleration": spaces.Box(-6.0, 6.0, shape=()),
        "steering": spaces.Box(-1.0, 1.0, shape=()),
    })
...
def action_factory(env: 'AbstractEnv', config: dict) -> ActionType:
  ...
  elif config["type"] == "L0Action":
    return L0Action(env, **config)
  else:
