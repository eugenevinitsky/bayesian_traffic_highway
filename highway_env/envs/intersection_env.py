...
class IntersectionEnv(AbstractEnv):
  ...
  def step(self, action: int) -> Tuple[np.ndarray, float, bool, dict]:
    ...
    info["agents_rewards"] = tuple(
        self._agent_reward(action, vehicle)
        for vehicle in self.controlled_vehicles)
    info["agents_dones"] = tuple(
        self._agent_is_terminal(vehicle)
        for vehicle in self.controlled_vehicles)
  ...
  def _make_vehicles(self, n_vehicles: int = 10) -> None:
    ...
    for ego_id in range(0, self.config["controlled_vehicles"]):
      ...
      if hasattr(ego_vehicle, "index_to_speed"):
      ...
      for v in self.road.vehicles:  # Prevent early collisions
        if v is not ego_vehicle and np.linalg.norm(v.position -
                                                   ego_vehicle.position) < 20:
  ...
  def _clear_vehicles(self) -> None:
    ...
    self.road.vehicles = [
        ...
        if vehicle in self.controlled_vehicles or
